...
class DockerManager():
	...
	def __init__(self, compiled_dir: str, debug_dir: str) -> None:
	...
	def build_for_checker(self, executable_file_name: str) -> tuple[str, bytes]:
		content = "\n".join([
			...
			f"COPY {executable_file_name} app/{executable_file_name}",
			f"RUN chown appuser:appgroup /app/{executable_file_name}",
			f"RUN chmod 500 /app/{executable_file_name}",
			f"USER appuser",
			f"CMD [ \"./app/{executable_file_name}\" ]\n"
		])
	...
	def build_for_debugger(self, executable_file_name: str) -> tuple[str, bytes]:
		content = "\n".join([
			...
			f"RUN chmod 500 /app/{executable_file_name}",
			f"USER appuser",
			f"CMD [ \"gdb /app/{executable_file_name}\" ]\n"
		])
		...
		try:
			stdout = subprocess.check_output(["docker", "build", "-t", self.debug_image_name, self.debug_dir])
			status = DckStatus.success
		except:
			status = DckStatus.docker_build_error
	...
	def run_for_debugger(self, input_: str, memory_limit_MB: int) -> tuple[subprocess.Popen, str]:
		container_name = str(uuid4())

		with open(f"{self.debug_dir}/input", "w") as f:
			f.write(input_)

		process = subprocess.Popen(["docker", "run", "-m", f"{memory_limit_MB}m", "--name", container_name, self.debug_image_name], stdout=subprocess.PIPE)

		return (process, f"{self.debug_dir}/input")
