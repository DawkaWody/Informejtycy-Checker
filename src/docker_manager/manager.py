...
class DockerManager():
	...
	def build_for_debugger(self, executable_file_name: str) -> tuple[str, bytes]:
		content = "\n".join([
			...
			f"RUN mkdir /usr/share/gcc/python/libstdcxx/v6/"							# Making directory for printers.py (gdb pretty print)
			f"RUN COPY {self.gdb_printers_dir}/printers.py /usr/share/gcc/python/libstdcxx/v6/printers.py" # Copying printers.py
			...
			f"RUN chmod 100 /app/a.out",												# Permissions: execute only
	...
	def run_for_debugger(self, input_: str, memory_limit_MB: int) -> tuple[pexpect.spawnu, str, str]:
		...
		return (process, container_name, f"{self.debug_dir}/input.txt")
