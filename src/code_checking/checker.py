...
from server import RECEIVED_DIR
	...
	def listen(self) -> None:
		...
		while True:
			if len(self.check_queue) > 0:
				...
				result = self.check(filename, client, ex_id)
				on_checked(result, client, ex_id)
				del self.check_queue[0]
				# JESLI BEDZIEPOTRZEBA, USUWANIE ZGLOSZENIA UZYTKOWNIKA
				# os.remove(os.path.join(RECEIVED_DIR, filename))

	def check(self, code_file: str, client: Client, ex_id: int) -> dict[str, Any]:
		"""
		...
		if ex_id >= self.pack_loader.get_problem_count():
